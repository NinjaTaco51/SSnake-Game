...
.data
frameBuffer: 	.space 	0x40f0		#((64 * 64) / 4) / 4
...
score:       .word 0        		# Stores the player's score

gameOverPrompt: .asciiz "Game Over! Your score was: "
...
loadSnake:
	jal init_queue		# Initialize the looping queue
	
#Generate first apple
	lw $t0, head 		# Load head byte offset
	add $t0, $s7, $t0 	# Get location of the head
	add $t0, $t0, -128	# Generate the first apple location
	li $t1, 0x00ff0000	# Load color red
	sw $t1, ($t0)		# Store red in memory (apple)
	
j gameUpdateLoop		# Start the game
...
gameUpdateLoop:
	...
	addi	$a0, $zero, 250	# frame rate (4 fps)
	syscall
...
add:	
	sw $t2, ($s1)	# Store the pixel of head into queue
	# End of enqueuing snake
	
	add $a0, $s5, $zero	# Store directions

	# Collision checking
	lw $t6, ($s1)		# Load the new head byte offset
...
printSnake:
	beqz $t5, gameUpdateLoop		# Update game once done printing the snake
	bgt $t2, $t8, resetQueuePointer	# Loop the queue
...
resetQueuePointer:
	la $t2, queue		# Loop the queue once it reaches the end of memory
	j continuePrinting

appleHead:	
	addi $s3, $s3, 1	# Add to snake size
 ...
 generateNewApple:
    	...
    	li $a1, 222		# Number of playable tiles (16x16) - (17 + 17)
    	syscall
    		
    	move $t0, $a0		# Store random number in $t0
    	add $t0, $t0, 17      	# Add to random number to not include top border
    	mul $t0, $t0, 4       	# Convert random index to byte offset (4 bytes per grid cell)
	add $t0, $s7, $t0	# Load the tile on grid
...
resetDirection:
	...
	#Added for scoring 
	sw $zero, score          # Reset score to 0
